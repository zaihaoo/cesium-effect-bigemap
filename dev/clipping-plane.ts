import { WGS84_POSITION } from '../Example/Types/index';
...
let _projections2Time: { [index: _PROJECTION_ID]: number } = {};
...
let _last_draw_assist_line: Array<Entity> = [];

class MultiClippingPlane {
	// 切割凹多边形
	static _cutting (_projection_positions: Cartesian3[], _riding_index: number[]) {
		const _riding: number[][] = [];
		// const _result: Cartographic[][] = [];
		const _result: {
			cartographic: Cartographic[][];
			mercator: Cartesian3[][];
			collineation_index: _COLLINEATION_ID[];
		} = { cartographic: [], mercator: [], collineation_index: [] };
		...
		while (_projection_ploygon.length > 0) {
			...
			projection_iterate.forEach((val, index) => {
				...
				if (Math.sign(i_cross) !== Math.sign(l_cross)) {
					...
					const t =
						((l1p1.y - l2p1.y) * (l2p1.x - l2p2.x) - (l2p1.y - l2p2.y) * (l1p1.x - l2p1.x)) /
			...
			// 保存拆分的线段
			_result.collineation_index.push(
				_collineation.push([
			...
			ploygon1.forEach((v, i) => {
				if (v === intersection_flag) {
					...
					if (
						intersection_position.x === projection_iterate[v].x &&
	...
	static _updatePoint (
		...
		_projection_positions: Cartesian3[],
		_draw_line: Entity[],
		position: Cartesian2 | Cartesian3,
		is_move = false, // 是否为鼠标移动捕获到的点
		...
		let cartesian_web_mercator: Cartesian3;
		let done = false;
		let effective = true;
		let draw = false;
		...
		if (!cartesian) return { effective: false, draw: draw, done: done };
		...
		if (
			...
			cartesian_web_mercator = _web_mercator.project(
				_viewer!.scene.globe.ellipsoid.cartesianToCartographic(cartesian)
			);
		}
		...
		if (_positions.length >= 2) {
			...
			const check_line = [
				new Cartesian2(
					_projection_positions[_projection_positions.length - 1].x,
					_projection_positions[_projection_positions.length - 1].y
				),
			...
			for (let i = 0; i < _projection_positions.length - 1; i++) {
				...
				if (i === _projection_positions.length - 2) {
					const collineation = MultiClippingPlane._isSegmentsCollineation(
						check_line[0],
						check_line[1],
						passivity_line[0],
						passivity_line[1]
					...
					const crossA1 = Cartesian2.cross(
						Cartesian2.subtract(check_line[1], check_line[0], new Cartesian2()),
						Cartesian2.subtract(passivity_line[0], check_line[0], new Cartesian2())
					);
					const crossA2 = Cartesian2.cross(
						Cartesian2.subtract(check_line[1], check_line[0], new Cartesian2()),
						Cartesian2.subtract(passivity_line[1], check_line[0], new Cartesian2())
					);

					if (Math.sign(crossA1) == 0) cross_is_zero++ && (crossA_is_diff = true);
					else if (Math.sign(crossA2) == 0) cross_is_zero++ && (crossA_is_diff = true);
					else if (Math.sign(crossA1) !== Math.sign(crossA2)) crossA_is_diff = true;

					// 满足不相交的条件
					if (!crossA_is_diff) continue;

					let crossB_is_diff = false;
					const crossB1 = Cartesian2.cross(
						Cartesian2.subtract(passivity_line[1], passivity_line[0], new Cartesian2()),
						Cartesian2.subtract(check_line[0], passivity_line[0], new Cartesian2())
					);
					const crossB2 = Cartesian2.cross(
						Cartesian2.subtract(passivity_line[1], passivity_line[0], new Cartesian2()),
						Cartesian2.subtract(check_line[1], passivity_line[0], new Cartesian2())
					);

					if (Math.sign(crossB1) == 0) cross_is_zero++ && (crossB_is_diff = true);
					else if (Math.sign(crossB2) == 0) cross_is_zero++ && (crossB_is_diff = true);
					else if (Math.sign(crossB1) !== Math.sign(crossB2)) crossB_is_diff = true;
					...
					if (cross_is_zero) {
						...
						if (cross_is_zero === 4) {
							...
							// 判断当前顶点是否为第一个顶点
							if (
								cartesian_web_mercator.x !== _projection_positions[0].x ||
		...
		_positions.length > 0 &&
			(draw = true) &&
			_draw_line.push(
				MultiClippingPlane._drawIntersectionLine(
					[_positions[_positions.length - 1], cartesian],
					effective ? Color.GREEN : Color.RED,
					is_move ? true : effective ? false : true
				)!
			);
	...
	static _clearDrawLine (_draw_line: Array<Entity>) {
		_draw_line.forEach(v => _viewer!.entities.remove(v));
	};

	static draw (positions?: _WGS84_POSITION[]) {
		// const aaa = [
		// 	[
		...
		return new Promise<_PROJECTION_ID>((resolve, reject) => {
			...
			let _draw_line: Array<Entity> = [];
			let _delete_flag = false;

			if (positions) {
				// batch
				const p = [...positions];
				let last_position: Cartesian3;
				let index = 0;
				while (p.length > 0) {
					const wgs84 = p.shift()!;
					index++;
					last_position = Cartesian3.fromRadians(wgs84.lon, wgs84.lat, 0);
					const _r = MultiClippingPlane._updatePoint(
						_positions,
					...
					if (!_r.effective) return reject(`第${index}个批量处理的顶点数据有误`);
				}
				const _r = MultiClippingPlane._updatePoint(
					_positions,
					_projection_positions,
				...
				_handler!.setInputAction(function (event: any) {
					// console.log(event);
					_delete_flag = MultiClippingPlane._updatePoint(
						_positions,
						_projection_positions,
						_draw_line,
						event.endPosition,
	...
	static async _done (
		_positions: Cartesian3[],
		_projection_positions: Cartesian3[],
		_riding_index: number[],
		resolve: (value: number | PromiseLike<number>) => void,
		reject: (reason?: any) => void,
		_draw_line: Array<Entity>
	) {
		console.log('@@@@');
		_status = _Mode.todo;
		_reject = undefined;
		...
		if (_positions.length >= 3) {
			...
			_projection_positions.forEach((val, index) => {
				...
				for (let i = 0; i < _projection_positions.length; i++) {
					...
					if (
						(l === last_index || l === index || i === last_index || i === index) &&
						_projection_positions.length !== 3
					)
						continue;
					const i_cross = Cartesian2.cross(
						Cartesian2.subtract(
							new Cartesian2(val.x, val.y),
							new Cartesian2(_projection_positions[last_index].x, _projection_positions[last_index].y),
							new Cartesian2()
						),
						Cartesian2.subtract(
							new Cartesian2(_projection_positions[i].x, _projection_positions[i].y),
					...
					const l_cross = Cartesian2.cross(
						...
						Cartesian2.subtract(
							new Cartesian2(_projection_positions[l].x, _projection_positions[l].y),
							new Cartesian2(_projection_positions[last_index].x, _projection_positions[last_index].y),
							new Cartesian2()
						)
					);
			...
			lengths.forEach((v, i) => {
				...
				for (let ii = s; ii < v + s; ii++) {
					plane_effective.push(
						Cartesian3.fromRadians(effective[ii].longitude, effective[ii].latitude, effective[ii].height)
					);
				}
				...
				_ploygons_boundingbox_backup.forEach((boundingbox, i) => {
					...
					polygonA.forEach((va, ia) => {
						...
						polygonB.forEach((vb, ib) => {
							...
							// 两条线段是否相交
							const intersection = MultiClippingPlane._segmentsIntersectionPoint(
								va,
								polygonA[ia_next],
								vb,
								polygonB[ib_next]
					...
					if (intersection_points.length >= 3) {
						...
						const origin = Cartesian2.divideByScalar(
							Cartesian2.add(
								Cartesian2.add(intersection_points[0], intersection_points[1], new Cartesian2()),
						...
						intersection_points.forEach(v => {
							...
							const sin_points2origin =
								v_origin.y / window.Math.sqrt(v_origin.x * v_origin.x + v_origin.y * v_origin.y);
						...
						intersection_polygon.forEach(
							(v, ii) =>
								(_intersectionLine2Projections[
									_intersection_line.push([
										v,
										intersection_polygon[(ii + 1) % intersection_polygon.length],
									]) - 1
								] = [_projection_index, _cutting2projection_positions[i]])
						);
			...
			Object.keys(positions.old).forEach(v => {
				...
				_projection2WallIndex[key] = [
					_wall.push(
						MultiClippingPlane._createWellWall(
							positions.old[key].position,
							positions.old[key].min_heights,
							positions.old[key].max_heights
						)
					) - 1,
				];
			});
			...
			const create_wall_index =
				_wall.push(MultiClippingPlane._createWellWall(positions.position, positions.min_heights, positions.max_heights)) - 1;
	...
	static _pointInsidePolygon (point: Cartesian3, polygon: Cartesian3[]) {
		// 这里如果叉乘为0 则点在多边形的顶点或者边上 这里如果结果为0直接认为在多边形外 因为后面计算两个多边形交点的时候会把该交点计算进去
		for (let i = 0; i < polygon.length; i++) {
			let next_i = (i + 1) % polygon.length;
			const i_cross = Cartesian2.cross(
				Cartesian2.subtract(
					new Cartesian2(polygon[next_i].x, polygon[next_i].y),
					new Cartesian2(polygon[i].x, polygon[i].y),
					new Cartesian2()
				),
				Cartesian2.subtract(
	...
	static _segmentsIntersectionPoint (a: Cartesian3, b: Cartesian3, c: Cartesian3, d: Cartesian3) {
	...
	// 删除裁剪面
	static async remove (_projection_id: _PROJECTION_ID) {
		if (_status == _Mode.undo) return;
		if (!_projections[_projection_id]) return;

		console.log('remove');
		_clippingPlaneCollections[_projection2Collection[_projection_id]].forEach(v =>
			_viewer!.scene.globe.multiClippingPlanes!.remove(v)
		);
		...
		_projection2WallIndex[_projection_id].forEach(
			v => _viewer!.scene.primitives.remove(_wall[v]) && delete _wall[v]
		);
		...
		// 备份分割凹多边形延长边的线段
		const _intersection_line_and_projection_line = _projection2Collineations[_projection_id].map(
			v => _collineation[v]
		);
		...
		const check_intersection_lines = [..._intersection_line].filter((v, i) => {
			return (
				...
				check_intersection_lines_attr_projection.push(
					_intersectionLine2Projections[i][1 - _intersectionLine2Projections[i].indexOf(_projection_id)]
				) &&
		...
		_intersection_line_and_projection_line.push(
			..._projection.map<[Cartesian2, Cartesian2]>((v, i) => [
				new Cartesian2(v.x, v.y),
				new Cartesian2(
					_projection[(i + 1) % _projection.length].x,
					_projection[(i + 1) % _projection.length].y
				),
			])
		);
		...
		const wall_obj: {
			[index: _PROJECTION_ID]: { position: Cartesian3[][]; max_heights: number[][]; min_heights: number[][] };
		} = {};
		...
		check_intersection_lines.forEach((v, i) => {
			...
			const line = MultiClippingPlane._getLerpObject(
				[new Cartesian3(v[0].x, v[0].y), new Cartesian3(v[1].x, v[1].y)],
				_intersection_line_and_projection_line
			);
		...
		Object.keys(wall_obj).forEach(async v => {
			...
			_projection2WallIndex[key].push(
				_wall.push(
					MultiClippingPlane._createWellWall(wall_obj[key].position, wall_obj[key].min_heights, wall_obj[key].max_heights)
				) - 1
			);
		});
	};
	...
	static async _ellipsoidToLonLat (c: _WGS84_POSITION[]) {
		// _reproject拆分多边形交点坐标然后重投影回cesium 笛卡尔坐标系的球面时使用的经验算法 所以高程有问题不能使用 需要重新获取
		// let alt = _viewer!.scene.globe.getHeight(cartographic);
		return await sampleTerrainMostDetailed(
			_viewer!.terrainProvider,
			c.map(v => new Cartographic(v.lon, v.lat))
		);
	};

	static _segmentsDifference (lineA_min: number, lineA_max: number, lineB_min: number, lineB_max: number) {
		// ab为线段A cd为线段B
	...
	static _isSegmentsCollineation (a: Cartesian2, b: Cartesian2, c: Cartesian2, d: Cartesian2) {
		// ab为线段A cd为线段B 线段A为绘制的线段 线段B为交集多边形的边
		// 由于在计算过程中存在精度问题 所以这里只能近似判断取值!!!!
		if (
			window.Math.abs(
				Cartesian2.cross(
					Cartesian2.subtract(b, a, new Cartesian2()),
					Cartesian2.subtract(d, c, new Cartesian2())
				)
			) > 0.00001
		)
			return [[a, b]];
		if (
			window.Math.abs(
				Cartesian2.cross(
					Cartesian2.subtract(c, a, new Cartesian2()),
					Cartesian2.subtract(d, a, new Cartesian2())
				)
			) > 0.00001
		)
			return [[a, b]];
	...
	static _getLerpObject (
		polygon: Cartesian3[],
		_intersection_line: [Cartesian2, Cartesian2][],
		is_bottom_surface: boolean = false
	) {
		const wgs84: _WGS84_POSITION[] = [];
		...
		if (polygon.length === 2) {
			...
			for (let ii = 0; ii < _intersection_line.length; ii++) {
				...
				seg_backup.forEach(v => {
					MultiClippingPlane._isSegmentsCollineation(v[0], v[1], _intersection_line[ii][0], _intersection_line[ii][1]).forEach(
						vv => seg.push(vv)
					);
				});
				if (!seg) break;
			...
			for (let i = 0; i < polygon.length; i++) {
				...
				for (let ii = 0; ii < _intersection_line.length; ii++) {
					...
					seg = [];
					seg_backup.forEach(v => {
						MultiClippingPlane._isSegmentsCollineation(
							v[0],
							v[1],
							_intersection_line[ii][0],
							_intersection_line[ii][1]
						).forEach(vv => seg.push(vv));
					});
					if (!seg) break;
				}
	...
	static async _getClippingAreaMinHeight (_projection_id: _PROJECTION_ID) {
	...
	static async _lerp (t: Cartesian3[], _need_update_polygons: number[], _projection_id: _PROJECTION_ID) {
	...
	static _createBottomSurface (e: number[]) {
	...
	static _createWellWall (position: Cartesian3[][], min_heights: number[][], max_heights: number[][]) {
	...
	static _drawDebugLine (v: [Cartesian2, Cartesian2] | [Cartesian3, Cartesian3], color: Color, dotted: boolean) {
		let entity: Entity;
		if (v[0] instanceof Cartesian2) {
			...
			entity = new Entity({
				...
				polyline: new PolylineGraphics({
					clampToGround: true,
					show: true,
					positions: v as [Cartesian3, Cartesian3],
					width: 7.5,
					material: dotted
						? (new PolylineDashMaterialProperty({
								color: color,
	...
	static _drawIntersectionLine (
		line: [Cartesian2, Cartesian2][] | Cartesian3[],
		color = Color.GREEN,
		dotted = false
	) {
		let entity: Entity;
		line.forEach((v, i) => {
			...
			if (!(v instanceof Array<Cartesian2>)) {
				if (i !== line.length - 1) {
					entity = MultiClippingPlane._drawDebugLine([v, line[i + 1] as Cartesian3], color, dotted);
				} else {
					return;
				}
			} else {
