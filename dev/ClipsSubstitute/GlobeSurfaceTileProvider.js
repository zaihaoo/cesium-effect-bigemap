...
GlobeSurfaceTileProvider.prototype.beginUpdate = function (frameState) {
  ...
  let multiClippingPlanes = this._multiClippingPlanes;
...
GlobeSurfaceTileProvider.prototype.computeTileVisibility = function (
  ...
  if (defined(multiClippingPlanes) && multiClippingPlanes.enabled) {
    for (let i = 0; i, multiClippingPlanes.length; i++) {
      let plane = multiClippingPlanes[i];
      let planeIntersection = plane.computeIntersectionWithBoundingVolume(
        boundingVolume
      );
      tile.isClipped = planeIntersection !== Intersect.INSIDE;
      if (planeIntersection === Intersect.OUTSIDE) {
...
function createTileUniformMap(frameState, globeSurfaceTileProvider) {
  const uniformMap = {
    ...
    u_clippingPlanes: function () {
      let multiClippingPlanes = globeSurfaceTileProvider._multiClippingPlanes;
      if (defined(multiClippingPlanes)) {
        let texture = multiClippingPlanes.dataTexture;
        if (defined(texture)) {
          return texture;
        }
      }
    ...
    u_clippingPlanesMatrix: function () {
      let multiClippingPlanes = globeSurfaceTileProvider._multiClippingPlanes;
      if (defined(multiClippingPlanes)) {
        let transform = Matrix4.multiply(
    ...
    u_clippingPlanesEdgeStyle: function () {
      let multiClippingPlanes = globeSurfaceTileProvider._multiClippingPlanes;
      if (defined(multiClippingPlanes)) {
        let style = multiClippingPlanes.edgeColor.clone();
    ...
    u_multiClippingPlanesLength: function () {
      let clippingPlanesLength = globeSurfaceTileProvider._multiClippingPlanes;
      if (defined(clippingPlanesLength)) {
        let texture = clippingPlanesLength.lengthTexture;
