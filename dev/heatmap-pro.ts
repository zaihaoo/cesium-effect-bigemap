...
export class Heatmap3D {
	...
	private _points?: { x: number; y: number; value: number; radius: number; lon_length: number; lat_length: number }[];
	...
	constructor(options: { fill?: boolean; source_points: { x: number; y: number; value: number }[]; alt?: number }) {
		this._delta = 100;
	...
	public readonly getTexture = () => {
		...
		this._dilatation_lon_length = this.getDistance(
		...
		this._dilatation_lat_length = this.getDistance(
		...
		const canvas = document.getElementsByClassName('heatmap-canvas') as HTMLCollectionOf<HTMLCanvasElement>;
	...
	// 创建纹理
	public readonly createTextures = (context: any) => {
		if (this._heatmap_texture) this._heatmap_texture.destroy();
		if (this._grey_texture) this._grey_texture.destroy();
	...
	public readonly update = (frame_state: any) => {
	...
	/* 计算两点之间的距离 */
	public readonly getDistance = (start: Cesium.Cartographic, end: Cesium.Cartographic) => {
		const geodesic = new Cesium.EllipsoidGeodesic();
		geodesic.setEndPoints(start, end);
		const distance = geodesic.surfaceDistance;
		return distance;
	};
	public readonly getPosition = (start: Cesium.Cartographic, end: Cesium.Cartographic, distance: number) => {
		const geodesic = new Cesium.EllipsoidGeodesic();
		geodesic.setEndPoints(start, end);
	...
	//用prototype给定方法和属性
	public readonly getCommand = (context: any) => {
		// 创建geometry
