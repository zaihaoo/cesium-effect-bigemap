...
type _Points = { x: number; y: number; value: number; radius: number; lon_length: number; lat_length: number }[];
...
export class Heatmap3D {
	...
	private _alt: number;
	private _arr: number[];
	private _heatmap: HTMLCanvasElement;
	private _grey: HTMLCanvasElement;
	private _indices: number[];
	private _st: number[];
	private _fill: boolean;
	private _source_points: { x: number; y: number; value: number }[];
	// 不能小于5
	private _radius: number;
	private _max_radius: number;
	private _points: _Points;
	private _data: { min: number; max: number; data: _Points };

	private _width: number;
	private _height: number;
	private _min_lon: number;
	private _min_lat: number;
	private _min_value: number;
	private _max_lon: number;
	private _max_lat: number;
	private _max_value: number;
	private _lon_length: number;
	private _lat_length: number;

	private _dilatation_width: number;
	private _dilatation_height: number;
	private _dilatation_min_lon: number;
	private _dilatation_min_lat: number;
	private _dilatation_max_lon: number;
	private _dilatation_max_lat: number;
	private _dilatation_lon_length: number;
	private _dilatation_lat_length: number;

	private _heatmap_instance: any;
	private _greymap_instance: any;
	private _heatmap_texture: any;
	private _grey_texture: any;
	private _point_command: any;

	constructor(options: {
		viewer: Cesium.Viewer;
		fill?: boolean;
		source_points: { x: number; y: number; value: number }[];
		alt?: number;
	}) {
		this._delta = 100;
		...
		this._initHeatmap();
		this._buildGemoetry();
		this._createTextures(this._viewer.scene.context);
		this._getCommand(this._viewer.scene.context);
	}

	private readonly _initHeatmap = () => {
		this._points = [];
		...
		this._lon_length = this._getDistance(
			Cesium.Cartographic.fromDegrees(this._min_lon, this._min_lat),
			Cesium.Cartographic.fromDegrees(this._max_lon, this._min_lat)
		);
		this._lat_length = this._getDistance(
			Cesium.Cartographic.fromDegrees(this._min_lon, this._min_lat),
			Cesium.Cartographic.fromDegrees(this._min_lon, this._max_lat)
		);
		...
		[this._dilatation_min_lon, this._dilatation_min_lat] = Object.values(
			this._pixel2position({ x: 0, y: this._dilatation_height })
		);
		[this._dilatation_max_lon, this._dilatation_max_lat] = Object.values(
			this._pixel2position({ x: this._dilatation_width, y: 0 })
		);
		this._dilatation_lon_length = this._getDistance(
			Cesium.Cartographic.fromDegrees(this._dilatation_max_lon, this._dilatation_min_lat),
			Cesium.Cartographic.fromDegrees(this._dilatation_min_lon, this._dilatation_min_lat)
		);
		this._dilatation_lat_length = this._getDistance(
			Cesium.Cartographic.fromDegrees(this._dilatation_min_lon, this._dilatation_max_lat),
			Cesium.Cartographic.fromDegrees(this._dilatation_min_lon, this._dilatation_min_lat)
		);
		...
		heatmap_div.setAttribute(
			'style',
			`width:${this._dilatation_width}px; height:${this._dilatation_height}px;display:none;`
		);
		document.body.appendChild(heatmap_div);

		const greymap_div = document.createElement('div');
		greymap_div.setAttribute('id', 'greymap');
		greymap_div.setAttribute(
			'style',
			`width:${this._dilatation_width}px; height:${this._dilatation_height}px;display:none;`
		);
		...
		this._source_points.forEach(v => {
			...
			const point = {
				lon_length: this._getDistance(
					Cesium.Cartographic.fromDegrees(v.x, this._dilatation_min_lat),
					Cesium.Cartographic.fromDegrees(this._dilatation_min_lon, this._dilatation_min_lat)
				),
				lat_length: this._getDistance(
					Cesium.Cartographic.fromDegrees(this._dilatation_min_lon, v.y),
					Cesium.Cartographic.fromDegrees(this._dilatation_min_lon, this._dilatation_min_lat)
				),
				x: pixel.x,
				y: pixel.y,
		...
		[this._heatmap, this._grey] = canvas as HTMLCollectionOf<HTMLCanvasElement>;
	};

	private readonly _buildGemoetry = () => {
		// build plane A
		const count = 350;
		...
		// vertexs.forEach((v, i) => {
		// 	if (v[0] === this._dilatation_lon_length || v[1] === this._dilatation_lat_length) return;
	...
	// 创建纹理
	private readonly _createTextures = context => {
	...
	public readonly update = frame_state => {
		if (!this._heatmap_texture || !this._grey_texture) {
			this._createTextures(frame_state.context);
		}
	...
	/* 计算两点之间的距离 */
	private readonly _getDistance = (start: Cesium.Cartographic, end: Cesium.Cartographic) => {
		const geodesic = new Cesium.EllipsoidGeodesic();
		geodesic.setEndPoints(start, end);
		const distance = geodesic.surfaceDistance;
		return distance;
	};
	private readonly _getPosition = (start: Cesium.Cartographic, end: Cesium.Cartographic, distance: number) => {
		const geodesic = new Cesium.EllipsoidGeodesic();
		geodesic.setEndPoints(start, end);
		return geodesic.interpolateUsingSurfaceDistance(distance);
	};

	private readonly _position2pixel = (position: { lon: number; lat: number }) => {
		return {
			x:
				(this._width *
					this._getDistance(
						Cesium.Cartographic.fromDegrees(position.lon, this._min_lat),
						Cesium.Cartographic.fromDegrees(this._min_lon, this._min_lat)
					)) /
					this._lon_length +
				this._delta,
			y:
				...
				this._height *
					(1 -
						this._getDistance(
							Cesium.Cartographic.fromDegrees(this._min_lon, position.lat),
							Cesium.Cartographic.fromDegrees(this._min_lon, this._min_lat)
						) /
							this._lat_length),
		};
	...
	private readonly _pixel2position = (pixel: { x: number; y: number }) => {
		// pixel 换算距离
	...
	public readonly updateRadius = (radius: number) => {
	...
	//用prototype给定方法和属性
	private readonly _getCommand = context => {
		// 创建geometry
		...
		// ! 创建图元绘制
		let geometry = Cesium.GeometryPipeline.computeNormal(
			new Cesium.Geometry({
		...
		const attribute_locations = Cesium.GeometryPipeline.createAttributeLocations(geometry);
