import { Cartesian3, Cesium3DTileset, Color, Math, Matrix3, Matrix4, Transforms, Cartesian4, TilesBuildingTextureFlood, TilesBuildingTextureNight, } from '../Lib/Cesium/index.js';
let _tileset;
let _color;
let _viewer;
let _style;
...
const _update3dtiles = (tileset, opt) => {
    if (!tileset) {
        alert('缺少模型！');
        return;
    }
    ...
    if (opt.translation) {
        Matrix4.multiplyByTranslation(mtx, new Cartesian3(opt.translation.x, opt.translation.y, opt.translation.z), mtx);
    }
...
const _setStyle = (style) => {
    if (typeof style !== 'number' || !(style in _Style) || _style === style)
        return;
    _style = style;
...
const _setColor = (color) => {
    if (!color || _color === color)
        return;
    _color = color;
    _style_flood.setUniform('u_color', new Cartesian4(...(_color
