...
import { Cartesian2, Color, HeightReference, ImageMaterialProperty, Rectangle } from 'cesium';
...
export function bmglHeatMap(viewer, data, config) {
	this.viewer = viewer;
...
bmglHeatMap.prototype.fromLatLngToPoint = function (latLng) {
	let y =
		((1 -
			Math.log(Math.tan((latLng.lat * Math.PI) / 180) + 1 / Math.cos((latLng.lat * Math.PI) / 180)) /
			Math.PI) /
			2) *
		256;
	...
	this.entity = this.viewer.entities.add({
		...
		rectangle: {
			...
			coordinates: Rectangle.fromDegrees(
				Math.max(
					base.bounds.min.lng -
					((base.bounds.max.lng - base.bounds.min.lng) * base.canvas.delta) / base.canvas.width,
					-180
				),
				Math.max(
					base.bounds.min.lat -
					((base.bounds.max.lat - base.bounds.min.lat) * base.canvas.delta) / base.canvas.height,
					-90
				),
				Math.min(
					base.bounds.max.lng +
